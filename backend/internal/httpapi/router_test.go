...
func TestRouterLeavesHealthPublicAndProtectsReconciliation(t *testing.T) {
	...
	router := httpapi.NewRouter(healthService, verifier, reconcileService, accountService, referenceService, providerProfileService, &recordingListingService{created: sampleListing()}, &recordingModerationReview{listing: sampleListing()}, discoveryService, contactChannelService, contactRevealService, nil, nil, nil, nil)
