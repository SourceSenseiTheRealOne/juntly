...
func NewRouter(service health.Service, verifier authn.Verifier, reconcileService ReconcileService, accountService AccountService, referenceService ReferenceService, providerProfileService ProviderProfileService, listingService ListingService, moderationListingService ModerationListingService, publicDiscoveryService PublicDiscoveryService, contactChannelService ContactChannelService, contactRevealService ContactRevealService, messagingService MessagingService, quotationService QuotationService) http.Handler {
	...
	mux.Handle("/api/v1/discovery/listings", NewPublicDiscoveryHandler(publicDiscoveryService))
	mux.Handle("/api/v1/public/listings/", NewPublicListingHandler(publicDiscoveryService))
	...
	mux.Handle("/api/v1/moderation/listings/", authn.RequireVerifiedIdentity(verifier, NewModerationListingHandler(moderationListingService)))
	mux.Handle("/api/v1/me/conversations", authn.RequireVerifiedIdentity(verifier, NewMessagingHandler(messagingService)))
	mux.Handle("/api/v1/me/conversations/", authn.RequireVerifiedIdentity(verifier, NewMessagingHandler(messagingService)))
	mux.Handle("/api/v1/me/notifications", authn.RequireVerifiedIdentity(verifier, NewMessagingHandler(messagingService)))
	mux.Handle("/api/v1/me/notifications/", authn.RequireVerifiedIdentity(verifier, NewMessagingHandler(messagingService)))
