import { createBookingReview } from "@/shared/api/generated";
import {
  authorizedHeaders,
  correlated,
  exact,
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const up = await createBookingReview({
      baseUrl,
      body: { bookingId: v.bookingId, rating: Number(v.rating), body: v.body },
