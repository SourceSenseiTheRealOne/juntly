...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    const upstream = await listNotifications({
      baseUrl,
...
function validList(
  value: NotificationsResponse | undefined,
): value is NotificationsResponse {
  return (
    !!value &&
    exact(value, ["notifications"]) &&
    Array.isArray(value.notifications) &&
    value.notifications.every(validNotification)
  );
}
function validNotification(
  ...
  return (
    !!value &&
    exact(value, ["id", "kind", "read", "createdAt"]) &&
    uuid(value.id) &&
    [
