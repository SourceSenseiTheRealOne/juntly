...
import type { Message, MessagesResponse } from "@/shared/api/generated";
import {
  authorizedHeaders,
  correlated,
  dateTime,
...
export async function GET(
  ...
  try {
    const upstream = await listConversationMessages({
      baseUrl,
      path: { conversationId },
...
export async function POST(
  ...
  if (!baseUrl) return unavailable(id);
  try {
    const upstream = await sendConversationMessage({
      baseUrl,
      path: { conversationId },
      body: { body: value.body.trim() },
      headers: authorizedHeaders(token, id),
    });
    if (
      upstream.error ||
...
function validList(
  value: MessagesResponse | undefined,
...
function validMessage(value: Message | undefined): value is Message {
  return (
    !!value &&
    exact(value, ["id", "conversationId", "senderId", "body", "createdAt"]) &&
    uuid(value.id) &&
